...
function transES5(code) {
    return core_1.transform(code, babelOpt).code;
}
function wap(filePath, opts) {
    const webRuntime = fs_extra_1.readFileSync(path_1.join(__dirname, './template/web.runtime.js'));
    return transES5(`import card from ${filePath};
...
function app(cardName, opts = {}) {
    return transES5(`if(!window.__SNC__) throw new Error('请优先引入 app snc')
