...
const shouldUseSourceMap = !!maraConf.sourceMap; // 压缩配置
...
module.exports = function ({
  ...
  const chunksEntry = getChunks(`src/view/${entry}/index.*.js`); // https://github.com/survivejs/webpack-merge
  ...
  const webpackConfig = merge(baseWebpackConfig, {
    ...
    plugins: [new webpack.DefinePlugin(config.build.env.stringified), // 使作作用域提升(scope hoisting)
    // https://medium.com/webpack/brief-introduction-to-scope-hoisting-in-webpack-8435084c171f
    new webpack.optimize.ModuleConcatenationPlugin(), // Minify the code.
    new UglifyJsPlugin({
      uglifyOptions: {
