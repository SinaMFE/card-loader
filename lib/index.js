  ...
  if (isApp) {
    ...
    promise = BundleCardAssets(filePath, cardModuleId++, cardName)
      .then(({ cardModuleId, cardName }) => {
        ...
        const staticFileArr = glob
          .sync(path.join(staticBuildFilePath, '**/*.*'))
          .map(posixPath2PlatformPath);
...
function posixPath2PlatformPath(pathString) {
  if (typeof pathString !== 'string') {
    throw new Error('传递参数非字符串');
  }

  return path.join(...pathString.split(path.posix.sep));
}
...
function platformPath2posixPath(pathString) {
  ...
  return path.posix.join(...pathString.split(path.sep));
}
